// /pages/hire/categories/frontend/index.tsx
import React from "react";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { ChevronLeft, Plus } from "lucide-react";
import HireCard from "../../_components/HireCard";
import { routes } from "@/lib/constants";
import RoundedTabs from "@/components/global/RoundedTabs";

export default function FrontendCategory() {
	const tabs = [
		{ routeKey: "mobile", name: "Mobile" },
		{ routeKey: "backend", name: "Backend" },
		{ routeKey: "frontend", name: "Frontend" },
		{ routeKey: "ui-ux", name: "UI/UX" },
	];

	const hireCardsData = [
		{
			title: "Swift Developer",
			route: "/hire/categories/swift",
			imageUrl: "",
			technologies: ["Swift", "React Native"],
			experience: "3 years",
			projects: 13,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
		{
			title: "Flutter Developer",
			route: "/hire/categories/flutter",
			imageUrl: "",
			technologies: ["Flutter"],
			experience: "2+ years",
			projects: 7,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
		{
			title: "Flutter & Angular Developer",
			route: "/hire/categories/flutter-angular",
			imageUrl: "",
			technologies: ["Flutter", "Angular"],
			experience: "3 years",
			projects: 19,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
		{
			title: "Swift & React Native Developer",
			route: "/hire/categories/swift-react-native",
			imageUrl: "",
			technologies: ["Swift", "React Native"],
			experience: "3 years",
			projects: 13,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
		{
			title: "Flutter Developer",
			route: "/hire/categories/flutter-2",
			imageUrl: "",
			technologies: ["Flutter"],
			experience: "2+ years",
			projects: 7,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
		{
			title: "Flutter & Angular Developer",
			route: "/hire/categories/flutter-angular-2",
			imageUrl: "",
			technologies: ["Flutter", "Angular"],
			experience: "3 years",
			projects: 19,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
		{
			title: "Swift & React Native Developer",
			route: "/hire/categories/swift-react-native-2",
			imageUrl: "",
			technologies: ["Swift", "React Native"],
			experience: "3 years",
			projects: 13,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
		{
			title: "Flutter Developer",
			route: "/hire/categories/flutter-3",
			imageUrl: "",
			technologies: ["Flutter"],
			experience: "2+ years",
			projects: 7,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
		{
			title: "Flutter & Angular Developer",
			route: "/hire/categories/flutter-angular-3",
			imageUrl: "",
			technologies: ["Flutter", "Angular"],
			experience: "3 years",
			projects: 19,
			price: "₦100,000",
			description:
				"Build your custom mobile application and upload to all mobile app stores.",
		},
	];

	return (
		<div className="flex flex-col gap-8">
			<div className="flex items-center justify-between gap-4 md:gap-[32px]">
				<Link
					href={routes.hire}
					className="flex items-center justify-center w-[32px] h-[32px] md:w-[42px] md:h-[42px] bg-primary-grey rounded-full"
				>
					<ChevronLeft />
				</Link>
				<Button>
					<Plus />
					Post a Job
				</Button>
			</div>
			<div>
				<p className="text-xl font-bold">Frontend Developer</p>
				<p className="text-sm font-normal text-primary-darkGrey">
					Hire a frontend developer to create user interfaces and improve the
					look and feel of your website or app.
				</p>
			</div>
			<div>
				<RoundedTabs
					basePath={routes.hire_categories}
					tabs={tabs}
				/>
			</div>
			<div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-4">
				{hireCardsData.map((card, index) => (
					<HireCard
						key={index}
						{...card}
					/>
				))}
			</div>
		</div>
	);
}
